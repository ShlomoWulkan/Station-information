...
- **אין כיוון פיזי בסגנונות.** `start`/`end`/`marginStart` ולא
  `left`/`right`/`marginLeft`, ובלי `textAlign: 'right'` — ברירת המחדל `auto`
  כבר מיישרת לתחילת הכיוון. RTL מחליף את הפיזיים, `textAlign` כולל, ולכן
  `'right'` מיישר **שמאלה**. גם סדר הילדים הוא סדר קריאה: הראשון יופיע מימין.
...
- **אין `verify=False`.** הקריאה ל-SIRI נושאת את המפתח. `moran` באמת לא שולח
  את תעודת הביניים שלו, והתיקון הוא `bash scripts/fetch-siri-ca.sh` +
  `SIRI_CA_BUNDLE` — לא כיבוי אימות. להריץ שוב בכל חידוש תעודה.
- **`/health` הוא המקום לבדוק אם המערכת חיה.** `siriReachable` ו-`siriReason`
  אומרים אם הקשר למשרד התחבורה עובד ולמה לא. הפער הזה — health שדיווח רק על
  GTFS — הוא מה שאיפשר ל-`/arrivals` להיות שבור בלי שאיש ידע.
- **כתובת שגויה ב-`.env` נראית כמו תקלת רשת.** `SIRI_BASE_URL` הצביע פעם על
  שם מארח שלא קיים, והתוצאה הייתה 502 אחרי ריטריים — בלי שום רמז לסיבה.
- **`/alerts` לא פעיל.** הכתובת לא מצביעה על פיד קיים, והוא מחזיר `[]`
  שמשמעותו "אין גישה" ולא "אין התראות". אין לחבר אותו לממשק.
